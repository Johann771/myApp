...
/**
 * Performance test for the Qualityrecords entity.
 */
public class QualityrecordsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all qualityrecords").get("/api/qualityrecords").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new qualityrecords")
                        .post("/api/qualityrecords")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"supplier\": \"SAMPLE_TEXT\"" + ", \"test2\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_qualityrecords_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created qualityrecords").get("${new_qualityrecords_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created qualityrecords").delete("${new_qualityrecords_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Qualityrecords entity").exec(scn);
